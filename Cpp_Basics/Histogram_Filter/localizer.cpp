	...
	// your code here
	 int rows = grid.size();
  	...
  	newGrid.resize(rows,vector<float>(cols));
  	 float beliefs_per_cell = (float)1.0/area; 
  	for(int row_idx = 0; row_idx<rows;row_idx++) 
    {
      	for(int col_idx = 0; col_idx<cols;col_idx++) 
        {
          	newGrid[row_idx][col_idx] = beliefs_per_cell; 
        }  
    }
	return newGrid;
}
  ...
  float blurring) 
{
  ...
  // your code here
  	...
  	for(int row_idx = 0; row_idx<rows;row_idx++) 
    {
      	for(int col_idx = 0; col_idx<cols;col_idx++) 
        {
          	cell = beliefs[row_idx][col_idx];
          	new_row = mod_local(row_idx+dx,cols);
              //(((row_idx%cols) + dx ) % cols);
          	new_col = mod_local(col_idx+dy,rows);
              //(((col_idx%rows) + dy ) % rows);
	...
	float p_miss) 
{
  	...
  	for(int row_idx = 0; row_idx<rows;row_idx++) 
    {
      	for(int col_idx = 0; col_idx<cols;col_idx++) 
        {
          	hit = (color == grid[row_idx][col_idx]); 
          	newGrid[row_idx][col_idx] = beliefs[row_idx][col_idx]*(hit*p_hit + (1-														hit)*p_miss); 
